...
if __name__ == "__main__":
    print("Testing Pose Estimation Utilities...")

    # Test Umeyama algorithm
    print("\n1. Testing Umeyama algorithm:")
    ...
    np.random.seed(42)
    source_points = np.random.randn(100, 3)

    # Transform to target
    target_points = s_gt * (R_gt @ source_points.T).T + t_gt

    # Add small noise
    target_points += np.random.randn(*target_points.shape) * 0.01

    # Estimate transformation
    R_est, t_est, s_est = umeyama_alignment(source_points, target_points)

    print(
        f"   Ground truth rotation error: {compute_rotation_error(R_est, R_gt):.4f} deg"
    )
    print(
        f"   Ground truth translation error: {compute_translation_error(t_est, t_gt):.4f}"
    )
    print(f"   Ground truth scale error: {compute_scale_error(s_est, s_gt):.4f}%")

    assert compute_rotation_error(R_est, R_gt) < 1.0, "Rotation error too large"
    assert compute_translation_error(t_est, t_gt) < 0.1, "Translation error too large"
    assert compute_scale_error(s_est, s_gt) < 1.0, "Scale error too large"

    # Test pose estimation from NOCS
    print("\n2. Testing pose estimation from NOCS:")

    # Create synthetic NOCS map
    H, W = 100, 100
    mask = np.zeros((H, W), dtype=bool)
    mask[30:70, 30:70] = True

    # Create NOCS coordinates (uniform in [0, 1])
    nocs_map = np.random.rand(H, W, 3)
    ...
    result = estimate_pose_from_nocs(
        nocs_map, depth, mask, K, use_ransac=False, min_points=10
    )
